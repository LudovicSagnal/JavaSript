...
let citationSelector = document.querySelector("p");
let auteurSelector = document.querySelector("h2");
let buttonSelector = document.querySelector("button");
let lastNumber;

function actu () {
    for(i=0 ; i<arrayOfQuotes.length ; i++) {
        let randomAnswer = arrayOfQuotes[Math.floor(Math.random() * arrayOfQuotes.length)];
        if(randomAnswer === lastNumber) {
            return actu();
        }else {
            citationSelector.textContent = randomAnswer [0];
            auteurSelector.textContent = randomAnswer [1];
            lastNumber = randomAnswer;
            return randomAnswer;
        }
    }
};
